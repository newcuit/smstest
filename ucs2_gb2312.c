...
/**************************************************************************************
* Description    : 定义UCS2转换表
**************************************************************************************/
static const uint16_t ucs2gb2312_tab[][2] =
{
    {0x00A4,0xA1E8},
    {0x00A7,0xA1EC},
    {0x00A8,0xA1A7},
...
/**************************************************************************************
* Description    : 定义UCS2编码表长度
**************************************************************************************/
#define UCS2_TAB_LEN (sizeof(ucs2gb2312_tab)/sizeof(ucs2gb2312_tab[0]))
...
*******************************************************************************************/
uint16_t ucs2_to_gb2312(uint8_t *gb2312, uint8_t *ucs2, int len)
{
    uint16_t ucs;
    int length = 0;
    int32_t mid, low, high;

    for (; len > 0; len = len - 2) {
        low = 0;
        high = UCS2_TAB_LEN - 1;
        ucs = *ucs2++;
        ucs = (ucs << 8) + (*ucs2++ & 0x00FF);
        ...
        while (low <= high) {
            mid = (low + high) / 2;
            if (ucs2gb2312_tab[mid][0] > ucs) high = mid - 1;
            else if (ucs2gb2312_tab[mid][0] < ucs) low =  mid + 1;
            else break;
        }
        if (low <= UCS2_TAB_LEN && high >= 0) {
            *gb2312++ = (uint8_t)(ucs2gb2312_tab[mid][1] >> 8);
            *gb2312++ = (uint8_t)(ucs2gb2312_tab[mid][1]);
        } else {
            *gb2312++ = 0xA1;
            *gb2312++ = 0xA1;
        }
